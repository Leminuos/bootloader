...
static uint8_t var_buffer[NVRAM_MAX_VAR_SIZE];
...
void create_variable_data(nvram_var_t* var, char* var_name, uint8_t* data, uint32_t total_var_size)
{
    uint32_t var_index = 0;
    uint32_t crc_stored = 0;

    var_index = 0;
    memset(var_buffer, 0, NVRAM_MAX_VAR_SIZE);
    memcpy(var_buffer, var, sizeof(nvram_var_t));
    var_index = sizeof(nvram_var_t);
    memcpy(&var_buffer[var_index], var_name, var->var_name_size);
    var_index = var_index + var->var_name_size;
    memcpy(&var_buffer[var_index], data, var->var_length);
    var_index = var_index + var->var_length;
    crc_stored = CalculateCRC(var_buffer, total_var_size);
    memcpy(&var_buffer[var_index], &crc_stored, sizeof(uint32_t));
}
...
nvram_state_t set_variable_data(char* var_name, uint8_t var_attr, uint8_t* data, uint32_t data_len)
{
    ...
    if (ret == NVRAM_SUCCESS)
    {
        ...
        if (var_temp.var_length != data_len)
        {
            diff_var_len = 1;
        }
        else
        {
            create_variable_data(&var_temp, var_name, data, total_var_size);

            if (var_offset + total_var_size + sizeof(uint32_t) <= SECTOR_SIZE)
            {
                nvram_load(base_offset, buffer, SECTOR_SIZE);
                memcpy(&buffer[var_offset], var_buffer, total_var_size + sizeof(uint32_t));
                nvram_store(base_offset, buffer, SECTOR_SIZE);
            }
            else
            {
                uint32_t left_size = SECTOR_SIZE - var_offset;
                uint32_t right_size = var_offset + total_var_size + sizeof(uint32_t) - SECTOR_SIZE;
                
                nvram_load(base_offset, buffer, SECTOR_SIZE);
                memcpy(&buffer[var_offset], var_buffer, left_size);
                nvram_store(base_offset, buffer, SECTOR_SIZE);
                
                base_offset = base_offset + SECTOR_SIZE;
                nvram_load(base_offset, buffer, SECTOR_SIZE);
                memcpy(buffer, &var_buffer[left_size], right_size);
                nvram_store(base_offset, buffer, SECTOR_SIZE);
            }
        }
    }
    ...
    if (ret != NVRAM_SUCCESS || diff_var_len == 1)
    {
        ...
        else
        {
            ...
            base_offset = base_offset + SECTOR_SIZE;
            memcpy(buffer, &var_buffer[left_size], right_size);
            nvram_store(base_offset, buffer, right_size);
        }
